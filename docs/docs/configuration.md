...
After editing `config.yml`, run `/eco reload` to apply messages and most runtime rules. Storage backends, cross-server mode and Redis topology, `account-loading.mode`, and `account-loading.lazy.cache.enabled` still require a restart.
:::
