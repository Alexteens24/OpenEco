...
public class BalanceCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command,
        ...
        if (args.length == 1 && sender instanceof Player player && service.hasCurrency(args[0])
                && (!sender.hasPermission("simpleeco.command.balance.others") || service.findByName(args[0]).isEmpty())) {
            if (!player.hasPermission("simpleeco.command.balance")) {
                messages.send(sender, "no-permission");
                return true;
            }
    ...
    public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command,
        ...
        if (args.length == 2 && sender.hasPermission("simpleeco.command.balance.others")) {
